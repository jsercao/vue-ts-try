...
export default {
  ...
  start() {
    ...
    mock.onGet("/todo/list").reply(config => {
      //  config 指 前台传过来的值
      let mockTodo = Todos.map((todo: any) => {
        // 重组 Todos数组，变成我们想要的数据
        return {
          id: todo.id,
          title: todo.title,
          count: todo.record.filter((data: any) => {
        ...
        setTimeout(() => {
          resolve([
            200,
            {
              status: 0,
              message: "",
              data: mockTodo // 返回状态为200，并且返回todos数据
            }
          ]);
        }, 200);
      });
    ...
    mock.onGet("/todo/listId").reply(config => {
      ...
      let todo = Todos.find((todo: any) => {
        return id && todo.id === id;
      });
      // todo.count (等待完成数目)等于 todo.record（代办事项列表下面未被选择的数据
      todo
        ? (todo.count = todo
            ? todo.record.filter((data: any) => {
